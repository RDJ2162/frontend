define([
  ...
  class HomeViewModel {
    constructor(params) {
      ...
      this.status = ko.observable(this.statusOptions[0].value);
      ...
      this.customerNameValid = ko.pureComputed(() => {
        ...
        if (!/^[a-zA-Z\s]+$/.test(name.trim())) {
          return { valid: false, message: "Customer name can only contain letters and spaces" };
        }
      ...
      this.creditCardValid = ko.pureComputed(() => {
        ...
        if (cc && cc.length > 0) {
          ...
          if (!/^\d{13,19}$/.test(cleanCC)) {
            return { valid: false, message: "Credit card number must be 13-19 digits" };
          }
      ...
      this.formValid = ko.pureComputed(() => {
        return this.customerNameValid().valid &&
      ...
      this.handleSubmit = async () => {
        ...
        if (!this.formValid()) {
          ...
          const errors = [];
          if (!this.customerNameValid().valid) errors.push(this.customerNameValid().message);
          if (!this.mobileValid().valid) errors.push(this.mobileValid().message);
          if (!this.emailValid().valid) errors.push(this.emailValid().message);
          if (!this.addressValid().valid) errors.push(this.addressValid().message);
          if (!this.aadhaarValid().valid) errors.push(this.aadhaarValid().message);
          if (!this.creditCardValid().valid) errors.push(this.creditCardValid().message);
          if (!this.balanceValid().valid) errors.push(this.balanceValid().message);
          
          this.addMessage('error', 'Validation Failed', 
        ...
        try {
          ...
          this.addMessage('confirmation', 'Success!', 
          ...
          this.addMessage('error', 'Submission Failed', 
            error.message || 'An unexpected error occurred while creating the customer.');
        } finally {
